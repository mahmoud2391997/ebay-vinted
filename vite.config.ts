    ...
    plugins: [
      ...
      mode === "development" && componentTagger(),
      {
        ...
        configureServer(server) {
          ...
          app.post("/ebay/identity/v1/oauth2/token", async (req, res) => {
            try {
              const appId = process.env.EBAY_APP_ID;
              const certId = process.env.EBAY_CERT_ID;

              console.log('Token request - App ID exists:', !!appId, 'Cert ID exists:', !!certId);
          ...
          app.get("/ebay/buy/browse/v1/item_summary/search", async (req, res) => {
            try {
          ...
          app.get("/finding", async (req, res) => {
            try {
              ...
              const queryString = new URLSearchParams(modifiedQuery as any).toString();
          ...
          console.log('API middleware registered for /api routes');
        }
      }
    ].filter(Boolean),
    resolve: {
