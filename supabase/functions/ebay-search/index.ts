...
async function searchEbay(token: string, params: SearchRequest) {
  ...
  if (params.soldItemsOnly) {
    ...
    const findingParams = new URLSearchParams({
      ...
      'paginationInput.pageNumber': String(Math.floor(offset / limit) + 1),
    ...
    if (params.filter) {
      const priceMatch = params.filter.match(/price:\[(\.\.|[^]]+)\]/);
      if (priceMatch) {
        const priceRange = priceMatch[1];
        if (priceRange.startsWith('..')) {
          const maxPrice = priceRange.substring(2);
          findingParams.append('itemFilter(1).name', 'MaxPrice');
          findingParams.append('itemFilter(1).value', maxPrice);
        } else if (priceRange.endsWith('..')) {
          const minPrice = priceRange.substring(0, priceRange.length - 2);
          findingParams.append('itemFilter(1).name', 'MinPrice');
          findingParams.append('itemFilter(1).value', minPrice);
        } else if (priceRange.includes('..')) {
          const [minPrice, maxPrice] = priceRange.split('..');
          findingParams.append('itemFilter(1).name', 'MinPrice');
          findingParams.append('itemFilter(1).value', minPrice);
          findingParams.append('itemFilter(2).name', 'MaxPrice');
          findingParams.append('itemFilter(2).value', maxPrice);
        }
      }
    }

    const url = `https://svcs.ebay.com/services/search/FindingService/v1?${findingParams.toString()}`;
    console.log('Searching eBay (sold items):', url);

    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'X-EBAY-SOA-SECURITY-APPNAME': Deno.env.get('EBAY_APP_ID') || '',
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Finding API search failed:', errorText);
      throw new Error(`eBay Finding API search failed: ${response.status}`);
    }

    const data = await response.json();
    
    // Transform Finding API response to match Buy API format
    ...
    return {
      total: parseInt(data.findItemsAdvancedResponse?.[0]?.paginationOutput?.[0]?.totalEntries?.[0] || '0'),
      limit: limit,
      offset: offset,
